...
def get_cifar10_dataloaders(args, validation_split=0.0, max_threads=10):
    ...
    if validation_split > 0.0:
        split = int(np.floor((1.0 - validation_split) * len(full_dataset)))
        train_dataset = torch.utils.data.Subset(full_dataset, range(0, split))
        val_dataset = torch.utils.data.Subset(full_dataset, range(split, len(full_dataset)))
        train_loader = torch.utils.data.DataLoader(
            train_dataset,
