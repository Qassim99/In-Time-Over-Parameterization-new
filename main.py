...
def count_nonzero_flops(model, input_size):
    model.eval()
    flops = 0
    hooks = []
    param_count = 0
    ...
    def conv_hook(self, input, output):
        ...
        sigma = (weight == 0).float().mean().item()

        param_count += (kernel_size ** 2 * in_channels * out_channels) * (1 - sigma)
        output_size = ((input[0].size(2) - kernel_size + 2 * padding) / stride + 1) ** 2 * out_channels
        flop_mults = (kernel_size ** 2 * in_channels * (1 - sigma)) * output_size
        flop_adds = (kernel_size ** 2 * in_channels * (1 - sigma) - 1) * output_size
        flops_conv = flop_mults + flop_adds + output_size  
    ...
    def linear_hook(self, input, output):
        ...
        param_count += (in_features * out_features) * (1 - sigma)
        flop_mults = in_features * (1 - sigma) * out_features
        flop_adds = (in_features * (1 - sigma) - 1) * out_features
        flops_linear = flop_mults + flop_adds + out_features 
...
def main():
    ...
    for i in range(args.iters):
        ...
        flops_non_zero, params = count_nonzero_flops(model, (1, 3, 32, 32)) 
        flops = count_flops(model, (1, 3, 32, 32)) 
        print_and_log('Nonzero parameters: {}'.format(nonzero_params))
        print_and_log('Nonzero parameters from function: {}'.format(params))
        print_and_log('FLOPs nonzero: {}'.format(flops_non_zero))
        print_and_log('FLOPs all: {}'.format(flops))
